...
// 大域変数
static TCHAR szWindowClass[] = _T("Win32AP041");
static TCHAR szTitle[] = _T("Win32AP041");
HINSTANCE	hInst;
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	case WM_KEYDOWN:
		if (wParam == VK_TAB) {
			if (GetKeyState(VK_DOWN) & 0x80)	// Ctrlキーが押されているか
			{
				MessageBox(hWnd, _T("Ctrl+Tab"), _T("メッセージ"), MB_OK);
				return 0;

			}
			if (GetKeyState(VK_UP) & 0x80)	// Ctrlキーが押されているか
			{
				MessageBox(hWnd, _T("Ctrl+Tab"), _T("メッセージ"), MB_OK);
				return 0;
